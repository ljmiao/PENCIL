...
parser.add_argument('--momentum', default=0.9, type=float, metavar='M',
                    help='momentum')
parser.add_argument('--weight_decay', '--wd', default=10e-4, type=float,
...
def main():
    ...
    testset = datasets.CIFAR10('./data', train=False, transform=transform_val)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (inputs, labels, _, _, _) in enumerate(trainloader):
            if gpu_status:
            ...
            loss=pencil_loss(outputs,labels)
            ...
            running_corrects += torch.sum(preds == torch.max(labels.detach(),1)[1] )
        scheduler.step()
        ...
        # validate
        test_loss, test_acc = validate(model, testloader, criterion, epoch, test_sizes)
        ...
        if test_acc > best_acc:
            best_acc = test_acc
            torch.save(model.state_dict(),os.path.join(args.out,'fine_tune_asym4_resnet32.pth'))
...
def pencil_loss(outputs,labels):
    # sfm=nn.Softmax(1)
    # pred=sfm(outputs)
